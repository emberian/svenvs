...
( cd "$here" && "$HOL" )
for d in pca specneg selfprover; do
  say "2. slice: $d"
  ( cd "$here/$d" && "$HOL" )
done
...
if [ "$full" = 1 ]; then
  ...
  crowns="svenvs_tower_unconditional svenvs_tower_with_prover_upgrade svenvs_tower_with_kernel_upgrade"
else
  # build only the pure trunk+prover theory, skip the Tier-2 kernel crown
  ( cd "$here/integration" && "$HOL" integrationTheory.uo )
  crowns="svenvs_tower_unconditional svenvs_tower_with_prover_upgrade"
fi
