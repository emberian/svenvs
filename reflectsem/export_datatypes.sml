(* =========================================================================
   ...
   Run (on persvati, in $CAKEMLDIR/semantics so .holpath resolves):
     cd ~/dev/CakeML/semantics && \
       ~/dev/HOL/bin/hol < ~/dev/svenvs/reflectsem/export_datatypes.sml
   Output: /tmp/reflectsem_datatypes.ml  (submit via place-submit.sh)
   ========================================================================= *)
...
val out = TextIO.openOut "/tmp/reflectsem_datatypes.ml";
...
fun try_emit_dt ty =
  let
    ...
    (* record accessors/fupds *)
    val () =
      List.app (fn cls =>
          emit (exportLib.pdefine cls)
...
val () = print "wrote /tmp/reflectsem_datatypes.ml\n";
