...
| **Toward an *in-place* self-upgradable compiler** (the running `cake` swapping its own compiler, heap intact, repeatedly): the reset model is sound by specialising CakeML's parametric eval-correctness at `ci'`; the in-place model generalises the eval-oracle invariant to a **per-generation compiler map** and proves a mid-run compiler swap preserves it. | **PROVED** (key lemma) | `selfUpgrade/evalUpgradeBScript.sml : swap_preserves_recorded_orac_wf_gen, recorded_orac_wf_gen_const`; reset corollary in `selfUpgrade/RESET_MODEL.md`. Remaining (per `selfUpgrade/README.md`): generalise `s_rel` + re-thread the `do_eval` simulation, and the runtime `Repl` exposure. |
