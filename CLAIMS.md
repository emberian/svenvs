...
| **One ouroboros generation, live:** the running program synthesizes a candidate `exp`, its own kernel proves `∀i. eval_n … = Rval [Litv (IntLit (6i+10))]` against the ported real semantics, a semantics-breaking candidate is refused, and the proved source is self-fed through `Repl.nextString` so the in-binary verified compiler installs and runs it natively; the verdict is computed by the installed code itself. Not built: accumulating generations, the self-model, gating the improver. | **RAN** (live Candle) | `scripts/reflectsem-live.sh --ouroboros`; `candle/ouroboros.ml` (`ouro_thm`, `ouro_reject_ok = true`, `ouro_native_out`, `verdict = "OUROBOROS_ONE_GEN_OK"`) |
