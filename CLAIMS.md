...
| A concrete `num`-indexed succession **run in the logic**: a decidable regime `gj_sound`, a forward-certified climbing succession `gj_succ`; every version is sound (genesis-only assumption), and the identity (non-strengthening) succession is unconditional; computed sanity. | **PROVED (computed)** | `genealogy/genealogyEvalScript.sml : gj_vouch_sound, gj_forward_certified, gj_all_versions_sound, gj_identity_succession_unconditional, gj_succession_runs` |
...
| A concrete testimony episode **run in the logic** (int-pole plant): a load-bearing self-witness `w_emb` (only `k=4` discharges admissibility) is attested → the gate installs the genuinely-wider policy; non-disclosure keeps the tight one; the adversarial cart stays boxed 30 ticks under *both*, while the bare plant crashes in 1 tick. Computed, not asserted. | **PROVED (EVAL)** | `embodiment/embodimentEvalScript.sml : w_emb_entails_admissible, attested_disclosure_installs, nondisclosure_inert_here, disclosure_widens_authority, bare_plant_crashes, disclosed_runs_safe, nondisclosed_runs_safe, episode_runs` |
