...
top-level `CLAIMS.md` (PROVED / TRUSTED-GLUE / UNTRUSTED / ASSUMED).
...
The generalisation moves from a FIXED policy predicate to an untrusted
...
The untrusted controller is **UNTRUSTED**: nothing about it is assumed; the
...
> that merely *believed* an untrusted certificate without re-checking would
