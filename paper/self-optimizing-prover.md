...
| **Toward in-place compiler self-upgrade** (running `cake` swaps its own compiler, heap intact): reset model sound by specialising the parametric eval-correctness at `ci'`; in-place model generalises the eval-oracle invariant to a **per-generation compiler map** and proves a mid-run swap preserves it. | **PROVED** (key lemma) | `selfUpgrade/evalUpgradeBScript.sml : swap_preserves_recorded_orac_wf_gen`; `selfUpgrade/{README,RESET_MODEL}.md`. Remaining: generalise `s_rel` + re-thread the `do_eval` simulation; runtime `Repl` exposure |
