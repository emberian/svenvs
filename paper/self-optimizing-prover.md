...
| **Layer B — the compiler swaps itself for a proven-better one:** that pass is **baked into CakeML's own compiler** (`bvl_to_bvi$compile_def`) and an **altered self-hosting root `cake.S` bootstrapped** that carries it — it runs, self-hosts (self-compiled `.S` is smaller), and the pass **fires** end-to-end (empty `Let` eliminated, output unchanged). `optimise_correct` is `DISK_THM`-clean. | **RAN** (altered root) + **PROVED** (the pass) | `compilerOpt/cakeml-bvl_opt.patch`, `compilerOpt/LAYERB.md` (built/validated on persvati) — **residual:** the full `compile_correct` re-composition is one FFI-trace subgoal short (honest, in LAYERB.md) |
