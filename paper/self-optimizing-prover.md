...
| **Toward in-place compiler self-upgrade** (running `cake` swaps its own compiler, heap intact): the eval-oracle invariant is generalised to a **per-generation compiler map** (`s_rel_gen`); a swap-then-eval generation preserves it **end to end** and preserves the observable result relation — an in-place compiler swap keeps eval semantics (one swap-generation). | **PROVED** (keystone) | `selfUpgrade/selfUpgradeEndToEndScript.sml : selfupgrade_eval_simulation_step, s_rel_gen_const, selfupgrade_collapses_to_eval_simulation` (+ `evalUpgradeB`). Remaining: multi-swap lift to whole-program `semantics_prog`; runtime core eval op + `Repl` exposure; re-bootstrap |
